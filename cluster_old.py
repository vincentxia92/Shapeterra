...
def cluster(coord_array, tri_array, v, simil):
    ...
    simil = np.sort(list(set(simil)))  # Remove duplicate entries in simil, order unordered set
    ...
    for l in range(len(simil)):
        ...
        print ("%d%% similarity complete. %d clusters found. Elapsed time is %s seconds."%(simil[l]*100, np.amax(newcluster_i), elapsedtime))
